...
export default {
  ...
  methods: {
    getCurrentTeacherId() {
      // 从路由参数获取教师ID
      return this.$route.query.userId || this.$parent.teacherInfo?.id
    },
    ...
    async loadExamStats() {
      try {
        ...
        const res = await axios.get(`http://localhost:8080/api/test/teacher-statistics/${teacherId}`)
        if (res.data.success) {
          ...
          stats.forEach(stat => {
            this.examStats.total += stat.count
            if (stat.status === '未安排') {
              this.examStats.unscheduled = stat.count
            } else if (stat.status === '已安排') {
              this.examStats.scheduled = stat.count
            } else if (stat.status === '已考试') {
              this.examStats.finished = stat.count
            }
          })
        }
      } catch (error) {
    ...
    async loadExamList() {
      try {
        ...
        const res = await axios.get(`http://localhost:8080/api/test/teacher-exams/${teacherId}`)
        if (res.data.success) {
          this.examList = (res.data.data || []).sort((a, b) => b.testId - a.testId) // 按ID从大到小排序
          // 检查并更新考试状态
    ...
    manageScore(exam) {
      // 通过父组件切换到成绩管理界面，并传递学科信息
      this.$emit('switch-to-scores', exam.courseName)
    },
    ...
    async saveExam() {
      ...
      this.$refs.examFormRef.validate(async (valid) => {
        ...
        try {
          const res = await axios.post('http://localhost:8080/api/test/schedule', {
